...
void process_input()
{
    ...
    while (SDL_PollEvent(&event))
    {
        ...
        case SDL_KEYDOWN:
            ...
            case SDLK_a:
                g_drops_position.x -= 1.0f;
                break;
            case SDLK_d:
                g_drops_position.x += 1.0f;
                break;
            case SDLK_i:
                g_drops_movement.y = -0.5f;
            default: break;
            }
        default:
            break;
...
void update()
{
    ...
    // --- COLLISION LOGIC --- //
    float distance_x = fabs(((g_drops_position.x + INIT_POS_DROPS.x) - INIT_POS_BEAKER.x) - (g_drops_size.x + INIT_SCALE_BEAKER.x) / 2.0f);
    float distance_y = fabs(((g_drops_position.y + INIT_POS_DROPS.y) - INIT_POS_BEAKER.y) - (g_drops_size.y + INIT_SCALE_BEAKER.y) / 2.0f);

    if (distance_x <= 0.0f && distance_y <= 0.0f)
    {
        COLLIDED = true;
        g_drops_scale.x = -2.0f;
        g_drops_scale.y = -2.0f;

    }
    if (COLLIDED) {
        
        g_drops_matrix = glm::scale(g_drops_matrix, g_drops_scale);
    }
